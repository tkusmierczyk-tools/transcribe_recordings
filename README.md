...
- `meeting.txt`: the conversation, one speaker turn per line (`[00:01:23] S1: ...`)
...
- Long recordings are split at pauses into parts of up to 28 min with speakers/timestamps
  ...
  parts heard there, so `S1` stays the same person. Someone who can't be matched gets a new
  label; if they might be an earlier speaker it has a `?`, and the transcript says who
  (`[S5? may be S1 or S3 ...]`). The overlap adds roughly 5–30% more audio to transcribe.
